  ...
  let content = `
    ...
    <div class="weatherBlock__topInfo__left">
        <div class="weatherBlock__topInfo__left__temperature">
            <h3>${Math.round(data.list[0].main.temp)}°</h3>
        </div>
...
const getDataFromInput = () => {
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    if (cityInput == "") alert("Input can't be empty!");
      ...
      let data = await returnDataFromAPI(cityInput);
      console.log(data);
      if (data.cod == 404) alert("There's no such city!");
      addToLocalStorage(cityInput, JSON.stringify(data));
      location.reload();
    }
  });
};
